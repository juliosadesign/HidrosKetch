...
function getCategoryAliases(category: string, kind: string) {
  ...
  if (category === "Fontes e reservaÃ§Ã£o") {
    return ["Fontes e reservatÃ³rios", "Fontes e reservaÃ§Ã£o"];
  }
...
function getKindLabel(item: ComponentCatalogItem) {
  ...
  if (item.kind === "pipe" || item.kind === "junction") return "TubulaÃ§Ã£o";
  if (item.kind === "accessory") return "AcessÃ³rio K";
  if (item.kind === "valve") return "VÃ¡lvula";
  if (item.kind === "pump") return "Bomba";
  if (item.kind === "instrument") return "Instrumento";
  if (item.kind === "label") return "AnotaÃ§Ã£o";
...
export function QuickComponentSearch({
  ...
  }, [activeCategory, searchTerm, compact]);
  ...
  const searchInput = (
    <div className="relative min-w-[240px] flex-1">
      <input
        ...
        className="h-10 w-full rounded-xl border border-slate-700 bg-slate-950/80 px-3 text-xs text-slate-100 outline-none transition placeholder:text-slate-500 focus:border-cyan-400/70 focus:ring-2 focus:ring-cyan-500/20"
      />

      {isOpen && (
        <div className="absolute left-0 right-0 top-[calc(100%+0.45rem)] z-50 max-h-72 overflow-y-auto rounded-2xl border border-slate-700 bg-slate-950/98 p-2 shadow-2xl shadow-slate-950/70 backdrop-blur">
            ...
            <div className="grid gap-1.5">
              {filteredComponents.map((component) => (
                <button
                  ...
                  className="flex items-center justify-between gap-3 rounded-xl border border-slate-800 bg-slate-900/80 px-3 py-2 text-left transition hover:border-cyan-400/60 hover:bg-slate-800"
                >
                  <span className="min-w-0 truncate text-xs font-semibold text-slate-100">
                    {component.name}
                  </span>
                  <span className="shrink-0 rounded-full border border-cyan-500/30 bg-cyan-500/10 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-cyan-200">
